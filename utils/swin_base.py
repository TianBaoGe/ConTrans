...
from utils.mmcv_custom import load_checkpoint
...
class SwinTransformer(BaseModule):
    ...
    def __init__(self,
                 ...
                 frozen_stages=-1,
                 use_checkpoint=True,
                 pretrained='./pths/swin_base_patch4_window7_224_22k.pth',
                 init_cfg=None):
...
class CBSwinTransformer(BaseModule):
    def __init__(self, size, embed_dim=128, cb_zero_init=True, cb_del_stages=1, **kwargs):
        ...
        for cb_idx in range(1):
            cb_module = _SwinTransformer(embed_dim=embed_dim, patch_size=size, **kwargs)
            if cb_idx > 0:
            ...
            if i >= self.cb_del_stages-1:
                ...
                for j in range(jrange):
                    if cb_inplanes[i + j] != cb_inplanes[i]:
                        ...
                        layer = Reduce(cb_inplanes[i + j], cb_inplanes[i])
                    else:
                        layer = nn.Identity()
...
class Swin_Base(nn.Module):
    def __init__(self, size, pretrained=None):
        super().__init__()
        self.backbone = CBSwinTransformer(size)
        self.backbone.init_weights()

    def forward(self, x):
        y = self.backbone(x)[0]
        c1, c2, c3, c4 = y[0], y[1], y[2], y[3]
        return c1, c2, c3, c4

if __name__ == '__main__':
    print('#### Test Case ###')
    from torch.autograd import Variable
    x = Variable(torch.ones(2, 3,64,64)).cuda()
    model = Swin_Tiny().cuda()
    print("Input shape:", x.shape)
    y = model(x)
    print(len(y))
    print('Output shape:',y[0][0].shape)
    print('Output shape:',y[0][1].shape)
    print('Output shape:',y[0][2].shape)
    print('Output shape:',y[0][3].shape)
